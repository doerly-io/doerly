...
export class OrderDetailsComponent implements OnInit {
  ...
  get canEnd(): boolean {
    return (
      ...
      (this.order?.status === EOrderStatus.AwaitingPayment && !this.order?.executorCompletionConfirmed && this.order?.executorId === this.profileId) ||
      (this.order?.status === EOrderStatus.AwaitingConfirmation && !this.order?.executorCompletionConfirmed && this.order?.executorId === this.profileId)
    );
  }
